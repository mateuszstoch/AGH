...
\begin{table}[H]
  \centering
  \caption{Zestawienie wartości $h_{\min}$ wyznaczonych empirycznie oraz minimalnej wartości błędów.}
  \begin{tabular}{lccc}
    \toprule
    Schemat & $E(h_{\min})$  & $h_{\min}$ empiryczne & $h_{\min}$ teoretuczne \\
    \midrule
    Różnica w przód   & $2.55e-08$ & $ 10^{-8}$ & \approx  $ 10^{-8}$ \\
    Różnica centralna & $6.22e-12$ & $ 10^{-12}$ & \approx  $ 10^{-12}$ \\
    \bottomrule
  \end{tabular}
  \label{tab:hmin}
\end{table}
...
Dla podpunktów (e) i~(b) weryfikację przeprowadzono przy użyciu biblioteki \texttt{decimal} z~precyzją 5 cyfr znaczących (\texttt{getcontext().prec = 5}), oraz int8.

\textbf{Podpunkt (e):} Dla $a = 126, b=124$:
\begin{itemize}
  \item wersja niestabilna: $(126 + 124)/2 = -3$ (błąd przepełnienia),
  \item wersja stabilna: $126+ (124 - 126)/2 = 125$ (wynik poprawny).
\end{itemize}

\textbf{Podpunkt (b):} Dla $x = 100.100$, $y = 100.099$:
\begin{itemize}
  \item wersja niestabilna: $x^2 - y^2 =  10020 - 10020  = 0.0$ (utrata cyfr znaczących),
  \item wersja stabilna: $(x-y)(x+y) = 0.001 \times 200.199 = 0.20020$ (wynik prawidłowy).
\end{itemize}
...
\begin{itemize}
  \item Metoda \textbf{(a) -- podwójna precyzja} osiąga bardzo mały błąd względny, rzędu $10^{-13}$–$10^{-14}$. Jest to wynik użycia 64-bitowego akumulatora, który minimalizuje błąd zaokrąglenia przy każdym dodawaniu.
  ...
  \item Metoda \textbf{(c) -- algorytm Kahana} osiąga zdecydowanie najmniejszy błąd względny niezależnie od $n$. Efektywnie kompensuje błąd zaokrąglenia kosztem dodatkowych operacji arytmetycznych.
